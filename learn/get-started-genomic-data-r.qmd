...
description: "Load, inspect, filter, and visualize a BIGr VCF while learning which steps require ploidy-aware interpretation."
categories:
  - Getting Started
  - Genotype Data
...
1. load the VCF bundled with BIGr;
2. identify fixed, INFO, FORMAT, and sample fields;
3. extract genotype, dosage, depth, allele-depth, and probability values;
4. visualize depth and apply a documented filter; and
5. distinguish universal VCF operations from ploidy-aware interpretation.

## What you need

- R 4.4.0 or newer;
- BIGr and its dependencies; and
- about 20 minutes.

If R or BIGr is not ready, complete [Install R for BIGr](install-r.qmd) first.
...
Load BIGr. The tutorial uses `vcfR`, a BIGr dependency, to read and extract VCF fields. It uses the `ggplot2` package installed in the setup guide to make a plot.
...
No output means that all three checks passed. If a check fails, return to the [R and BIGr setup guide](install-r.qmd).

## 2. Load the BIGr example VCF
...
## 3. Understand the VCF structure
...
## 4. Inspect genotypes, dosage, and depth
...
## 6. Filter with an explicit scientific rule
